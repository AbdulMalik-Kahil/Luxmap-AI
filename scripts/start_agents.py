#!/usr/bin/env python3
"""Google Maps Agent Startup Script.

This script provides a convenient way to start and manage the
Google Maps A2A service. It checks for required API keys,
monitors agent health, and ensures graceful shutdown.

USAGE:
- Start the Maps agent:
  $ python start_agents.py

FEATURES:
- Uses `click` for a clean command-line interface.
- Loads environment variables from `.env`.
- Checks for required GOOGLE_MAPS_API_KEY.
- Starts the agent in a subprocess.
- Monitors agent health by checking the agent card endpoint.
- Logs agent output to files in the logs/ directory.
- Gracefully terminates the process on exit (Ctrl+C).
"""
import asyncio
import os
import subprocess
...
@click.command()
def main() -> None:
    """Starts and manages the Google Maps A2A agent service."""
    
    agents_to_start = ["maps"]
