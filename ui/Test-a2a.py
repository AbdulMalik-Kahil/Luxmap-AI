"""
Streamlit UI for the Google Maps Agent using A2A polling architecture.

This UI communicates with the Maps Agent as a decoupled service via the A2A SDK,
following the A2A polling pattern.
...
# Configuration
AGENT_URL = os.getenv("MAPS_AGENT_A2A_URL", "http://localhost:8002")
...
async def create_a2a_client() -> A2AClient | None:
    ...
    try:
        ...
        st.info("To start the maps agent server: `python -m maps_agent --port 8002`")
...
async def run_agent_logic_a2a(prompt: str) -> Dict[str, Any]:
    """
    Send a message to the Maps Agent via A2A and poll for results.
    
    Args:
        prompt: User's input message
        
    Returns:
        Dictionary containing final response, tool calls, and any artifacts
    """
    try:
        # Create A2A client
        ...
        if not client:
            return {
                'final_response': "❌ Failed to connect to Maps Agent service",
                'tool_calls': [],
                'tool_responses': [],
                'audio_url': None,
                'success': False
...
def main():
    ...
    st.set_page_config(
        page_title="Google Maps Agent (A2A)",
        page_icon="🗺️",
        layout="wide"
    )
    ...
    # Main UI
    st.title("🗺️ Google Maps Agent (A2A Architecture)")
    st.markdown("Search for locations and get information from Google Maps via A2A protocol.")
    ...
    # Chat input
    if prompt := st.chat_input("Search for locations on Google Maps..."):
