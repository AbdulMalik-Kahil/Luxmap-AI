"""Google Maps Agent Executor for A2A integration."""
...
MAPS_A2A_APP_NAME = "maps_a2a_app"


class MapsADKAgentExecutor(AgentExecutor):
    """ADK Agent Executor for Google Maps A2A integration."""
    ...
    def __init__(self, agent: Agent, agent_card: AgentCard, runner: Runner):
        ...
        Args:
            agent: The Google Maps ADK agent instance
            agent_card: Agent card for A2A service registration
            runner: Pre-configured ADK Runner instance
        """
        logger.info(f"Initializing MapsADKAgentExecutor for agent: {agent.name}")
        self.agent = agent
    ...
    def _prepare_input(self, context: RequestContext) -> str:
        ...
        if not user_input:
            ...
            user_input = "Search for nearby places"

        logger.info(f"{self.agent.name} processing location query: '{user_input}'")
        return user_input
    ...
    def _send_response(
        ...
        logger.info(f"Sending Google Maps search response for task {context.task_id}")
    ...
    def _handle_error(
        ...
        logger.error(
            f"Error executing Google Maps search in {self.agent.name}: {str(error)}",
            exc_info=True,
        )
        error_message_text = f"Error searching Google Maps: {str(error)}"
        event_queue.enqueue_event(
            new_agent_text_message(
    ...
    async def cancel(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        logger.info(
            f"Cancelling Google Maps search task: {task_id} for agent {self.agent.name}"
        )
        ...
        logger.info(f"Sent cancel event for Google Maps task: {task_id}")